...
int avlTree::adverbCounter()
{
	return adc;
}


int avlTree::verbCOunter()
{
	return verc;
}

int avlTree::avlCounter()
{
	return counter;
}
...
// Returns: Nothing.
//*************************************************************************
void avlTree::insert(string word, string wordtype) {
	node *newNode;

	newNode = new node(word, wordtype);
	insert(root, newNode);
...
bool avlTree::search(string word) {
	...
	while (nodePtr) {
		if (nodePtr->value == word) {
			counter++;
			if (nodePtr->key == "adjective")
				adjc++;
			else if (nodePtr->key == "adverb")
				adc++;
			else if (nodePtr->key == "verb")
				verc++;
			else if (nodePtr->key == "noun")
				nouc++;
			return true;
		}
