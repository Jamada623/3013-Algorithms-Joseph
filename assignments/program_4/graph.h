...
public:
	...
	*/
	void addEdge(int fromID, int toID, double weight = 0, bool directed = false)
	{
		...
		vertexList[fromID]->E.push_back(edge(toID,weight));
		num_edges++;
	...
	*/
	...
	void createSpanningTree(string filter = "")
	{
		ofstream outfile;
		ifstream infile;
		infile.open("filtered_cities.csv");
		double distance, mindistance,  totaldistance=0.0;
		double lat1, lat2, lon1, lon2;
		int minID = 0, edgecounter=0;
		int ID1,ID2;
		string civ1, civ2, cit;
		bool found;
		vector<vertex *>::iterator vit;
		vector<vertex *>::iterator vit1;
		vector<edge>::iterator eit;		
		infile >> cit;
		infile.close();
		for (vit = vertexList.begin(); vit != vertexList.end(); vit++)
		{
			...
			for (int i = 0; i < 3; i++) {
				...
				for (vit1 = vertexList.begin(); vit1 != vertexList.end(); vit1++)
				{
					...
					// if filter is not empty do the below
					
						if (ID1 != ID2 && (*vit1)->visited == false)
						{
							lat2 = (*vit1)->loc.lat;
							lon2 = (*vit1)->loc.lon;
							civ2 = (*vit1)->city;
							distance = distanceEarth(lat1, lon1, lat2, lon2);
							...
							if (distance < mindistance)
							{
								...
								(*vit1)->visited = true;
								minID = (*vit1)->ID;//keeps min distance upto date
								found = true;
								edgecounter += 1;
							}
						}
				...
				if (found == true)
				{
					addEdge(ID1, ID2, mindistance, false);
				}
			}
		...
		cout << " Total edges " << edgecounter;
		outfile << cit << endl;
		outfile << "Total distance " << totaldistance << endl;
		outfile << " Total edges " << edgecounter;
